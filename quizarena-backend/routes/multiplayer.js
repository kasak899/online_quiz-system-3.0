...
// QuizArena — Multiplayer module (REST + WebSocket)
...
class Player {
  constructor(ws, name, pid) {
    this.ws = ws; this.name = name; this.id = pid;
    this.score = 0; this.isHost = false;
    this.currentAnswer = null; this.answerTime = null;
  }
}

class Room {
  constructor(code, category) {
    this.code = code; this.category = category;
    this.players = new Map();
    this.state = "lobby";
    this.questions = []; this.currentQIndex = -1; this.qStartTime = null;
  }
  playerList() {
  ...
  broadcast(msg) {
    ...
    for (const p of this.players.values()) {
      try { if (p.ws.readyState === 1) p.ws.send(payload); } catch (e) {}
    }
  }
}

const rooms = new Map();
const createRoom = (cat) => {
  let code = randCode();
  while (rooms.has(code)) code = randCode();
  const r = new Room(code, cat);
  rooms.set(code, r);
  return r;
};
...
router.post("/room/create", (req, res) => {
  const cat = (req.body && req.body.category) || "coding";
  if (!QUIZ_BANK[cat]) return res.json({ error: "Invalid category. Use 'coding' or 'gk'" });
  const r = createRoom(cat);
  res.json({ code: r.code, category: r.category });
});

router.get("/room/:code", (req, res) => {
  const r = getRoom(req.params.code);
  if (!r) return res.json({ error: "Room not found" });
  res.json({ code: r.code, category: r.category, state: r.state, players: r.playerList() });
});
...
async function runGame(room) {
  ...
  for (let i = 0; i < questions.length; i++) {
    ...
    for (const p of room.players.values()) { p.currentAnswer = null; p.answerTime = null; }
    ...
    while (Date.now() < endTime) {
      const allAnswered = room.players.size > 0 &&
    ...
    room.broadcast({ type: "question_result", correct_index: correct, leaderboard: room.leaderboard() });
    await sleep(3000);
  }
...
function handleConnection(ws, code, name) {
  ...
  if (room.state !== "lobby") { ws.send(JSON.stringify({ type: "error", message: "Game already started" })); ws.close(); return; }
  ...
  ws.on("message", (raw) => {
    ...
    if (t === "start_game" && player.isHost && room.state === "lobby") {
      if (room.players.size >= 1) runGame(room);
    } else if (t === "answer" && room.state === "playing") {
  ...
  const cleanup = () => {
    ...
    if (player.isHost && room.players.size > 0) {
      room.players.values().next().value.isHost = true;
    }
    if (room.players.size > 0) {
      room.broadcast({ type: "player_left", name: player.name, players: room.playerList() });
    } else {
...
function attachWebSocket(server) {
  ...
  server.on("upgrade", (request, socket, head) => {
    ...
    if (!match) return; // let other upgrade handlers process it
    const code = match[1].toUpperCase();
    const name = url.searchParams.get("name") || "Player";
    wss.handleUpgrade(request, socket, head, (ws) => handleConnection(ws, code, name));
  });
  console.log("✅ Multiplayer WebSocket attached on /ws/multiplayer/{code}");
}
